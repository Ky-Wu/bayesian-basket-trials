...
alpha2 <- 0.05
# basket sizes (set to config of MEM optimal design in first study)
n_i <- c(4, 4, 4, 4)
n_b <- c(19, 19, 19, 19)
# interim threshold (reject if total Stage 1 <= r)
r <- 1
...
LMEM2_ef <- function(n_b, y, p0) LMEM2BasketEfficacy(n_b, y, p0, a = 1, b = 1,
                                                     d1 = d1, d2 = d2)
res <- computeESSFixedSS(p0, p1, n_i, n_b, r,
                         LMEM2_ef, alpha2, n_sim = n_sim)
LMEM2_res <- evaluateTwoStageScenarios(n_i,
                                       n_b,
                                       scenarios, p0 = p0,
                                       res$interim_threshold,
                                       res$pp_threshold,
                                       LMEM2_ef, n_sim = n_sim)
model_fp <- format_model_name(d1, d2)
saveRDS(LMEM2_res, file.path(getwd(), "output", "fixedss_sim", model_fp))
